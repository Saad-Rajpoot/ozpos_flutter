...
Presentation: Riverpod (or Bloc) + GoRouter; module UIs (POS, KDS, Dispatch, Reservations, Printers, Settings).
...
Data: Repositories (abstract), Local DB (Isar/Drift/SQLite), KV cache (Hive), Remote (Dio + interceptors: auth, retry, ETag).
...
State: Riverpod (deterministic, testable).
HTTP: Dio (+ retry/backoff, auth, logging in dev).
Models: Freezed + JsonSerializable.
Storage: Isar (or Drift) + Hive (KV); SharedPreferences for non-sensitive flags; flutter_secure_storage for tokens.
Routing: GoRouter (Navigator 2).
...
Sanctum tokens (device-scoped); CSRF for Web; strict CORS; Security Headers middleware (HSTS, CSP, X-Frame-Options, X-Content-Type-Options).
