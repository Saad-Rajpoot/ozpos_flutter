...
void main() async {
  ...
  final sentryDsn = 'https://5043c056bceb3ca2e4a92d2e6e2b0235@o4509604948869120.ingest.us.sentry.io/4510112203341824';
  ...
  final envelopeUrl = 'https://o4509604948869120.ingest.us.sentry.io/api/$projectId/envelope/';
  ...
  final event = {
    ...
    'exception': {
      'values': [
        {
          'type': 'TestException',
          'value': '🔥 MANUAL TEST ERROR - OZPOS Sentry Integration Verification from Dart Script',
          'stacktrace': {
            'frames': [
              {
    ...
    'message': {
      'formatted': '🧪 OZPOS Manual Test - This error was sent directly to verify Sentry integration is working'
    },
    'extra': {
  ...
  final envelope = '${json.encode(eventHeader)}\n${json.encode(itemHeader)}\n${json.encode(event)}';
  ...
  try {
    ...
    final response = await http.post(
      ...
      headers: {
        ...
        'X-Sentry-Auth': 'Sentry sentry_version=7, sentry_key=$publicKey, sentry_client=manual-test/1.0.0',
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      print('');
      print('If you see this error in your dashboard, the integration is working! 🎉');
    } else {
      print('');
