...
import '../../bloc/addon_management/addon_management_state.dart';
...
class AddonRulesCompactDisplay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AddonManagementBloc, AddonManagementState>(
      builder: (context, state) {
        ...
        final hasSpecificRules = sizeId != null &&
        ...
        final attachedCategoryIds = effectiveRules.map((r) => r.categoryId).toSet();
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Tooltip(
                  message: 'Click on any available set below to add it. All items are included by default.',
                  child: Icon(
                    Icons.help_outline,
                    size: 16,
            ...
            if (effectiveRules.isNotEmpty)
              _buildAttachedSetsList(context, state, effectiveRules, hasSpecificRules)
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.add_circle_outline, size: 16, color: Colors.green.shade700),
            ),
            const SizedBox(width: 8),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildAttachedSetsList(
    ...
    return Column(
      ...
      children: [
        ...
        ...rules.map((rule) => _buildExpandableRuleCard(context, state, rule)).toList(),
      ],
    );
  }
      ...
      child: OutlinedButton.icon(
        onPressed: () {
          context.read<AddonManagementBloc>().add(
                ResetSizeRulesToItemLevelEvent(
                  itemId: itemId,
                  sizeId: sizeId!,
                ),
              );
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: _buildRuleField(
                  ...
                  onChanged: (value) => _updateRule(
                    context,
                    rule.copyWith(minSelection: value),
                  ),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: _buildRuleField(
                  ...
                  onChanged: (value) => _updateRule(
                    context,
                    rule.copyWith(maxSelection: value),
                  ),
                ),
              ),
            ],
          ...
          SwitchListTile(
            ...
            onChanged: (value) => _updateRule(
              context,
              rule.copyWith(isRequired: value),
            ),
          ),
        ],
      ),
        ...
        if (rule.defaultItemIds.isNotEmpty)
          ...rule.defaultItemIds.map((addonItemId) {
            final item = category.items.firstWhere(
              (i) => i.id == addonItemId,
              orElse: () => const AddonItem(id: '', name: 'Unknown', basePriceDelta: 0),
            );
  ...
  Widget _buildAddItemsButton(
    ...
    return OutlinedButton.icon(
      onPressed: () => _showAddItemsDialog(context, category, rule, unselectedItems),
  ...
  void _attachCategoryWithAllItems(BuildContext context, AddonCategory category) {
  ...
  void _removeItemFromRule(
    ...
    final newItemIds = rule.defaultItemIds.where((id) => id != addonItemId).toList();
    final newOverrides = Map<String, double>.from(rule.priceOverrides)..remove(addonItemId);
