...
class AddonItemSelectorDialog extends StatefulWidget {
  ...
  @override
  State<AddonItemSelectorDialog> createState() => _AddonItemSelectorDialogState();
...
class _AddonItemSelectorDialogState extends State<AddonItemSelectorDialog> {
      ...
      child: Container(
        ...
        child: BlocBuilder<AddonManagementBloc, AddonManagementState>(
          builder: (context, state) {
            ...
            final validation = state.getValidation(widget.itemId, widget.categoryId);
            ...
            return Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Expanded(
                  child: _buildItemList(context, state, category, rule),
                ),
                const SizedBox(height: 16),
  ...
  Widget _buildItemList(
    ...
    final filteredItems = _searchQuery.isEmpty
        ? category.items
        : category.items.where((item) => item.name.toLowerCase().contains(_searchQuery)).toList();
        ...
        Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final displayPrice = rule.priceOverrides[item.id] ?? item.basePriceDelta;
      ...
      trailing: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () => _showPriceOverrideDialog(context, item, displayPrice),
          ),
        ],
      ),
    );
  ...
  Widget _buildFooter(BuildContext context, AddonManagementLoaded state, AddonRule rule) {
    return Row(
      children: [
        TextButton(
          onPressed: () {
  ...
  void _showPriceOverrideDialog(BuildContext context, AddonItem item, double currentPrice) {
  ...
  void _onSave(BuildContext context, AddonManagementLoaded state, AddonRule rule) {
