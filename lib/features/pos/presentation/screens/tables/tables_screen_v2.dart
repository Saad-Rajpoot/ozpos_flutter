...
class TablesScreenV2 extends StatefulWidget {
  const TablesScreenV2({super.key});

  @override
  State<TablesScreenV2> createState() => _TablesScreenV2State();
}

class _TablesScreenV2State extends State<TablesScreenV2> {
  ...
  final List<TableData> _mockTables = [
    ...
    const TableData(number: 5, status: TableStatus.available, area: TableArea.main),
    const TableData(number: 7, status: TableStatus.reserved, area: TableArea.main, guests: 4),
    const TableData(number: 9, status: TableStatus.cleaning, area: TableArea.main),
    const TableData(number: 11, status: TableStatus.available, area: TableArea.tens),
    const TableData(number: 13, status: TableStatus.occupied, area: TableArea.tens, serverTag: 'CD', amount: 45.30, elapsedTime: Duration(minutes: 35), guests: 3),
    const TableData(number: 21, status: TableStatus.available, area: TableArea.twenties),
    const TableData(number: 23, status: TableStatus.occupied, area: TableArea.twenties, serverTag: 'EF', amount: 88.90, elapsedTime: Duration(minutes: 55), guests: 5),
    const TableData(number: 101, status: TableStatus.available, area: TableArea.patio),
    const TableData(number: 103, status: TableStatus.reserved, area: TableArea.patio, guests: 6),
  ];
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildAreaItem(TableArea.main, 'Main', Icons.restaurant, _getAreaTableCount(TableArea.main)),
          _buildAreaItem(TableArea.tens, '10s', Icons.deck, _getAreaTableCount(TableArea.tens)),
          _buildAreaItem(TableArea.twenties, '20s', Icons.weekend, _getAreaTableCount(TableArea.twenties)),
          _buildAreaItem(TableArea.patio, 'Patio', Icons.outdoor_grill, _getAreaTableCount(TableArea.patio)),
        ],
      ),
    );
  }

  Widget _buildAreaItem(TableArea area, String label, IconData icon, int count) {
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: isActive ? Colors.white.withValues(alpha: 0.2) : const Color(0xFFF3F4F6),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    _buildFilterChip('Available', _filterStatus == TableStatus.available, () => setState(() => _filterStatus = TableStatus.available)),
                    const SizedBox(width: 8),
                    _buildFilterChip('Occupied', _filterStatus == TableStatus.occupied, () => setState(() => _filterStatus = TableStatus.occupied)),
                    const SizedBox(width: 8),
                    _buildFilterChip('Reserved', _filterStatus == TableStatus.reserved, () => setState(() => _filterStatus = TableStatus.reserved)),
                  ],
                ),
              ],
            ),
          ...
          children: [
            ...
            if (table.amount != null) ...[
              ...
              Text(
                ...
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: cardColor),
              ),
              ...
              Row(
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: cardColor),
                  ),
                ],
              ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Details grid
          _buildDetailRow('Items', '${table.items ?? 0}', 'Total', '\$${table.amount?.toStringAsFixed(2) ?? '0.00'}'),
          const SizedBox(height: 16),
          _buildDetailRow('Time', table.elapsedTime != null ? '${table.elapsedTime!.inMinutes.toString().padLeft(2, '0')}:${(table.elapsedTime!.inSeconds % 60).toString().padLeft(2, '0')}' : '--', 'Waiter', table.waiter ?? '--'),
          const SizedBox(height: 16),
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              _buildActionButton('Move Table', Icons.swap_horiz, false, _onMoveTable),
              _buildActionButton('Merge Tables', Icons.call_merge, false, _onMergeTables),
              _buildActionButton('Split Bill', Icons.call_split, false, _onSplitBill),
              _buildActionButton('Close Bill', Icons.check_circle, true, _onCloseBill),
              _buildActionButton('Edit Order', Icons.edit, false, _onEditOrder),
              _buildActionButton('Clear Table', Icons.clear, false, _onClearTable, isDanger: true),
            ],
          ),
        ],
      ),
      ...
      children: [
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(label1, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF6B7280))),
              const SizedBox(height: 4),
              Text(value1, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: Color(0xFF111827))),
            ],
          ),
        ),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(label2, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF6B7280))),
              const SizedBox(height: 4),
              Text(value2, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: Color(0xFF111827))),
            ],
          ),
        ),
      ],
  ...
  Widget _buildActionButton(String label, IconData icon, bool isPrimary, VoidCallback onTap, {bool isDanger = false}) {
    return SizedBox(
  ...
  void _onMoveTable() async {
    ...
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => MoveTableScreen(sourceTable: _selectedTable!),
      ),
    );
    ...
    if (result != null && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Table moved successfully'),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('Merge table ${_selectedTable!.number} - Feature coming soon'),
                  behavior: SnackBarBehavior.floating,
                ),
              );
            },
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  ...
                  style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Color(0xFF3B82F6)),
                ),
              ],
            ),
            const SizedBox(height: 16),
  ...
  void _onEditOrder() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Edit order for Table ${_selectedTable!.number} - Opening menu...'),
        behavior: SnackBarBehavior.floating,
