...
class _MoveTableScreenState extends State<MoveTableScreen> {
  ...
  final List<TableData> _mockTables = [
    const TableData(number: 1, status: TableStatus.available, area: TableArea.main, serverTag: '10s'),
    const TableData(number: 2, status: TableStatus.occupied, area: TableArea.main, serverTag: '10s', amount: 45.30, elapsedTime: Duration(minutes: 35), guests: 3),
    const TableData(number: 4, status: TableStatus.available, area: TableArea.main, serverTag: 'MAIN'),
    const TableData(number: 5, status: TableStatus.reserved, area: TableArea.main, guests: 4),
    const TableData(number: 6, status: TableStatus.available, area: TableArea.main, serverTag: '20s'),
    const TableData(number: 7, status: TableStatus.cleaning, area: TableArea.main),
    const TableData(number: 8, status: TableStatus.available, area: TableArea.patio, serverTag: 'PATIO'),
  ];
  ...
  Widget build(BuildContext context) {
    final canConfirm = _selectedDestination != null && _canSelectTable(_selectedDestination!);
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            ...
            const Text('Move Table', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
          ],
        ),
      ),
      body: Column(
                          ...
                          Row(
                            children: [
                              ...
                              Container(
                                ...
                                child: Text(
                                  widget.sourceTable.area.toString().split('.').last.toUpperCase(),
              ...
              children: [
                ...
                PopupMenuButton<TableArea?>(
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text(_filterArea == null ? 'All Areas' : _filterArea.toString().split('.').last),
                  ...
                  itemBuilder: (context) => [
                    ...
                    ...TableArea.values.map((area) => PopupMenuItem(
                      value: area,
  ...
  Widget _buildDestinationCard(TableData table) {
    ...
    return InkWell(
      onTap: canSelect ? () => setState(() => _selectedDestination = table) : null,
      borderRadius: BorderRadius.circular(12),
      child: Opacity(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: cardColor),
              ),
              if (table.amount != null) ...[
                const SizedBox(height: 4),
                Text(
                  '\$${table.amount!.toStringAsFixed(2)}',
                  style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: cardColor),
                ),
              ],
              if (table.guests != null)
                Text(
                  '${table.guests} guests',
                  style: TextStyle(fontSize: 11, color: cardColor.withValues(alpha: 0.7)),
                ),
            ],
          ),
        ),
  ...
  void _confirmMove() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Moving Table ${widget.sourceTable.number} to Table ${_selectedDestination!.number}'),
