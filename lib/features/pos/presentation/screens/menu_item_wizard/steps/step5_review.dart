          ...
          child: Column(
            ...
            children: [
              ...
              _buildReviewSection(
                ...
                stepNumber: 1,
                content: [
                  _buildReviewRow('Name', state.item.name.isNotEmpty ? state.item.name : 'Not set'),
                  if (state.item.description.isNotEmpty)
                    _buildReviewRow('Description', state.item.description),
                  if (state.item.sku.isNotEmpty) _buildReviewRow('SKU', state.item.sku),
                  _buildReviewRow('Category', state.item.categoryId.isNotEmpty ? state.item.categoryId : 'Not set'),
                  if (state.item.badges.isNotEmpty)
                    _buildReviewRow(
                      'Badges',
                      state.item.badges.map((b) => b.label).join(', '),
                    ),
                  if (state.item.displayImagePath != null && state.item.displayImagePath!.isNotEmpty)
              ...
              _buildReviewSection(
                ...
                stepNumber: 4,
                content: [
                  _buildReviewRow('Dine-In', state.item.dineInAvailable ? 'Available' : 'Not available'),
                  _buildReviewRow('Takeaway', state.item.takeawayAvailable ? 'Available' : 'Not available'),
                  _buildReviewRow('Delivery', state.item.deliveryAvailable ? 'Available' : 'Not available'),
                  if (state.item.prepTimeMinutes != null && state.item.prepTimeMinutes! > 0)
                    _buildReviewRow('Prep Time', '${state.item.prepTimeMinutes} minutes'),
                  if (state.item.kitchenStation != null && state.item.kitchenStation!.isNotEmpty)
                    _buildReviewRow('Kitchen Station', state.item.kitchenStation!),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: () {
                    context.read<MenuEditBloc>().add(NavigateToStep(stepNumber));
                  },
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildPriceInfo('Dine-In', size.dineInPrice),
              ),
              Expanded(
                child: _buildPriceInfo('Takeaway', size.takeawayPrice),
              ),
              Expanded(
                child: _buildPriceInfo('Delivery', size.deliveryPrice),
              ),
            ],
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                if (item.hasSizes && item.sizes.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 12),
                  ...item.sizes.map((size) => Container(
                    margin: const EdgeInsets.only(bottom: 8),
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(
                      color: size.isDefault ? const Color(0xFFEFF6FF) : Colors.white,
                      border: Border.all(
                        color: size.isDefault ? const Color(0xFF3B82F6) : Colors.grey.shade300,
                ...
                if (item.hasSizes && item.sizes.isNotEmpty) ...[
                  ...
                  ...item.sizes.where((s) => s.isDefault).expand((size) {
                    ...
                    return groupedAddOns.entries.expand((entry) => [
                      Text(
                        entry.key,
                        style: const TextStyle(
                          fontSize: 15,
                      ...
                      const SizedBox(height: 12),
                      ...entry.value.map((addon) => Container(
                        margin: const EdgeInsets.only(bottom: 8),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (item.upsellItemIds.isNotEmpty) ...[
                          const SizedBox(height: 12),
                          Row(
                            children: item.upsellItemIds.take(2).map((id) => Expanded(
                              child: Container(
                        ...
                        if (item.relatedItemIds.isNotEmpty) ...[
                          ...
                          const SizedBox(height: 12),
                          Row(
                            children: item.relatedItemIds.take(2).map((id) => Expanded(
                              child: Container(
                                margin: const EdgeInsets.only(right: 8),
                                padding: const EdgeInsets.all(12),
                                decoration: BoxDecoration(
