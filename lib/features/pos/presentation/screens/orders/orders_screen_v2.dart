...
import '../../../data/datasources/mock_orders_data.dart';
...
class _OrdersScreenV2State extends State<OrdersScreenV2> {
  ...
  List<OrderEntity> get _filteredOrders {
    return _allOrders.where((order) {
      ...
      // Channel filter
      if (_selectedChannels.isNotEmpty && !_selectedChannels.contains(order.channel)) {
        return false;
      }
  ...
  Map<String, dynamic> get _stats {
    final active = _allOrders.where((o) => o.status == OrderStatus.active).length;
    final completed = _allOrders.where((o) => o.status == OrderStatus.completed).length;
    final cancelled = _allOrders.where((o) => o.status == OrderStatus.cancelled).length;
      ...
      child: Scaffold(
        ...
        body: Row(
          children: [
            ...
            if (isDesktop)
              const SidebarNav(activeRoute: AppRouter.orders),
  ...
  Widget _buildHeader() {
    return Container(
      decoration: const BoxDecoration(
        color: Colors.white,
        border: Border(
          bottom: BorderSide(color: OrdersTokens.colorBorder),
        ),
      ),
      child: Padding(
      ...
      children: [
        Expanded(
          child: _buildKPICard(
            ...
            gradientColors: const [OrdersTokens.colorKpiActiveStart, OrdersTokens.colorKpiActiveEnd],
          ),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _buildKPICard(
            ...
            gradientColors: const [OrdersTokens.colorKpiCompletedStart, OrdersTokens.colorKpiCompletedEnd],
          ),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _buildKPICard(
            ...
            icon: Icons.cancel,
            gradientColors: const [OrdersTokens.colorKpiCancelledStart, OrdersTokens.colorKpiCancelledEnd],
          ),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _buildKPICard(
            ...
            gradientColors: const [OrdersTokens.colorKpiRevenueStart, OrdersTokens.colorKpiRevenueEnd],
          ),
        ),
      ],
    );
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(10),
          borderSide: const BorderSide(color: OrdersTokens.colorKpiActiveStart, width: 2),
        ),
        ...
        child: Row(
          children: [
            Icon(
              icon,
              size: 14,
              color: isActive ? OrdersTokens.colorTextPrimary : OrdersTokens.colorTextMuted,
            ),
