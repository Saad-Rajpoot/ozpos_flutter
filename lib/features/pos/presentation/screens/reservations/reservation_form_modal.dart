...
class _ReservationFormModalState extends State<ReservationFormModal> {
  ...
  void initState() {
    ...
    _partySizeController = TextEditingController(text: res?.party.size.toString() ?? '2');
  ...
  DateTime get _endDateTime => _fullDateTime.add(Duration(minutes: _durationMinutes));
                      ...
                      child: Form(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildTextField('Guest Name *', _nameController, 'Enter full name', validator: _requiredValidator),
                            const SizedBox(height: 16),
                            Row(
                              children: [
                                Expanded(child: _buildTextField('Phone', _phoneController, '+1 XXX-XXX-XXXX')),
                                const SizedBox(width: 16),
                                Expanded(child: _buildTextField('Email', _emailController, 'guest@example.com')),
                              ],
                            ),
                            ...
                            Row(
                              children: [
                                Expanded(child: _buildTextField('Party Size *', _partySizeController, '0', keyboardType: TextInputType.number, validator: _partySizeValidator)),
                                const SizedBox(width: 16),
                            ...
                            TextFormField(
                              ...
                              decoration: InputDecoration(
                                hintText: 'Special requests, dietary restrictions, etc.',
                                filled: true,
        ...
        DropdownButtonFormField<int>(
          ...
          items: [60, 90, 120, 150, 180, 240].map((mins) {
            return DropdownMenuItem(
              value: mins,
              child: Text('${(mins / 60).toStringAsFixed(mins % 60 == 0 ? 0 : 1)}h'),
            );
          }).toList(),
  ...
  Widget _buildTagsSection() {
    final availableTags = ['Birthday', 'Anniversary', 'Allergy', 'VIP', 'Accessible'];
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _buildSummaryItem('Guest', _nameController.text.isEmpty ? '—' : _nameController.text),
                const SizedBox(height: 16),
                _buildSummaryItem('Date & Time', '${dateFormat.format(_fullDateTime)}\n${timeFormat.format(_fullDateTime)}'),
                const SizedBox(height: 16),
                _buildSummaryItem('Party Size', _partySizeController.text.isEmpty ? '—' : '${_partySizeController.text} guests'),
                const SizedBox(height: 16),
                _buildSummaryItem('Duration', '${(_durationMinutes / 60).toStringAsFixed(_durationMinutes % 60 == 0 ? 0 : 1)} hours'),
                const SizedBox(height: 16),
                _buildSummaryItem('End Time', timeFormat.format(_endDateTime)),
                if (_selectedTags.isNotEmpty) ...[
                  const SizedBox(height: 16),
                  _buildSummaryItem('Tags', _selectedTags.map((t) => t[0].toUpperCase() + t.substring(1)).join(', ')),
                ],
              ],
            ),
          ),
  ...
  void _saveReservation() {
    if (_formKey.currentState!.validate()) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(widget.reservation == null ? 'Reservation created successfully' : 'Reservation updated successfully'),
