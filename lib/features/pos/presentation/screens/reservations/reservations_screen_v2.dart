...
class ReservationsScreenV2 extends StatefulWidget {
  const ReservationsScreenV2({super.key});

  @override
  State<ReservationsScreenV2> createState() => _ReservationsScreenV2State();
}

class _ReservationsScreenV2State extends State<ReservationsScreenV2> {
  ...
  void _loadMockData() {
    ...
    _reservations = [
      ReservationEntity(
        ...
        timing: ReservationTiming(startAt: today.add(const Duration(hours: 19)), durationMinutes: 90),
      ...
      ReservationEntity(
        ...
        timing: ReservationTiming(startAt: today.add(const Duration(hours: 19, minutes: 30)), durationMinutes: 90),
      ...
      ReservationEntity(
        ...
        timing: ReservationTiming(startAt: today.add(const Duration(hours: 18)), durationMinutes: 120),
      ...
      ReservationEntity(
        ...
        branchId: 'branch-1',
        tableId: 'Table 10',
        guest: const GuestInfo(name: 'Linda Martinez', phone: '+1 234-567-8905'),
        party: const PartyDetails(size: 2),
        timing: ReservationTiming(startAt: today.add(const Duration(hours: 18, minutes: 30)), durationMinutes: 90),
      ...
      ReservationEntity(
        ...
        party: const PartyDetails(size: 4),
        timing: ReservationTiming(startAt: tomorrow.add(const Duration(hours: 19)), durationMinutes: 90),
        status: ReservationStatus.confirmed,
        source: ReservationSource.website,
        preferences: const ReservationPreferences(tags: ['allergy']),
        financials: const ReservationFinancials(depositAmount: 50.0, depositStatus: DepositStatus.held),
        audit: AuditInfo(createdBy: 'system', createdAt: now),
      ),
      ReservationEntity(
        ...
        timing: ReservationTiming(startAt: tomorrow.add(const Duration(hours: 20)), durationMinutes: 120),
        status: ReservationStatus.confirmed,
        source: ReservationSource.phone,
        preferences: const ReservationPreferences(tags: []),
        financials: const ReservationFinancials(),
  ...
  Widget build(BuildContext context) {
    ...
    return MediaQuery(
      ...
      child: Scaffold(
        ...
        body: Row(
          children: [
            if (isDesktop) const SidebarNav(activeRoute: AppRouter.reservations),
            Expanded(
              child: Column(
                children: [
                  _buildHeader(),
                  _buildToolbar(),
                  Expanded(child: _isListView ? _buildListView() : _buildFloorView()),
                ],
              ),
            ),
          ],
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                _buildViewButton('List View', Icons.view_list, _isListView, () => setState(() => _isListView = true)),
                _buildViewButton('Floor View', Icons.grid_view, !_isListView, () => setState(() => _isListView = false)),
              ],
            ),
          ),
          ...
          // Date chips
          _buildDateChip('Today', _selectedDateFilter == 'Today', () => setState(() => _selectedDateFilter = 'Today')),
          const SizedBox(width: 8),
          _buildDateChip('Tomorrow', _selectedDateFilter == 'Tomorrow', () => setState(() => _selectedDateFilter = 'Tomorrow')),
          const SizedBox(width: 8),
          _buildDateChip('All', _selectedDateFilter == 'All', () => setState(() => _selectedDateFilter = 'All')),
        ],
      ),
    );
  }

  Widget _buildViewButton(String label, IconData icon, bool isSelected, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border(
            right: label == 'List View' ? const BorderSide(color: Color(0xFFE5E7EB)) : BorderSide.none,
          ),
        ),
        child: Row(
          children: [
            Icon(icon, size: 18, color: isSelected ? const Color(0xFF3B82F6) : const Color(0xFF6B7280)),
            const SizedBox(width: 8),
            Text(
              label,
              style: TextStyle(
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: const Row(
                children: [
                  Expanded(flex: 2, child: Text('GUEST NAME', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF6B7280)))),
                  Expanded(child: Text('DATE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF6B7280)))),
                  Expanded(child: Text('TIME', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF6B7280)))),
                  Expanded(child: Text('PARTY SIZE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF6B7280)))),
                  Expanded(child: Text('TABLE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF6B7280)))),
                  Expanded(child: Text('STATUS', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF6B7280)))),
                  Expanded(flex: 2, child: Text('ACTIONS', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF6B7280)), textAlign: TextAlign.right)),
                ],
              ),
            ),
            
            // Table rows
            ...reservations.map((reservation) => _buildReservationRow(reservation)),
          ],
        ),
      ),
    );
  ...
  List<Widget> _buildActionButtons(ReservationEntity reservation) {
    ...
    if (reservation.canSeat) {
      buttons.add(_buildActionButton('Seat', const Color(0xFF10B981), () => _seatReservation(reservation)));
    }

    if (reservation.canCheckout) {
      buttons.add(_buildActionButton('Checkout', const Color(0xFF3B82F6), () => _checkoutReservation(reservation)));
    }

    buttons.add(IconButton(
      icon: const Icon(Icons.edit, size: 18),
      onPressed: reservation.canEdit ? () => _editReservation(reservation) : null,
      tooltip: 'Edit',
    ));

    buttons.add(TextButton(
      onPressed: reservation.canCancel ? () => _cancelReservation(reservation) : null,
      child: const Text(
        'Cancel',
  ...
  void _seatReservation(ReservationEntity reservation) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Seating ${reservation.guest.name} at ${reservation.tableId}'),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              setState(() {
                final index = _reservations.indexWhere((r) => r.reservationId == reservation.reservationId);
                if (index != -1) {
                  _reservations[index] = reservation.copyWith(status: ReservationStatus.cancelled);
                }
              });
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
