...
class AddonManagementBloc
    extends Bloc<AddonManagementEvent, AddonManagementState> {
  ...
  Future<void> _onLoadCategories(
    ...
    try {
      ...
      emit(AddonManagementLoaded(categories: categories, itemAttachments: {}));
    } catch (e) {
  ...
  Future<void> _onUpdateRule(
    ...
    final validationResults =
        Map<String, Map<String, AddonRuleValidation>>.from(
  ...
  Future<void> _onToggleItemSelection(
    ...
    for (var i = 0; i < itemAttachments.length; i++) {
      if (itemAttachments[i].sizeId == event.sizeId) {
        final rules = List<AddonRule>.from(itemAttachments[i].rules);
        final ruleIndex = rules.indexWhere(
          (r) => r.categoryId == event.categoryId,
        );
  ...
  Future<void> _onCreateCategory(
    ...
    emit(
      currentState.copyWith(categories: categories, hasUnsavedChanges: true),
    );
  }
    ...
    final categories = currentState.categories.map((c) {
      if (c.id == event.categoryId) {
        ...
        for (var i = 0; i < event.itemIds.length; i++) {
          final item = c.items.firstWhere(
            (item) => item.id == event.itemIds[i],
          );
