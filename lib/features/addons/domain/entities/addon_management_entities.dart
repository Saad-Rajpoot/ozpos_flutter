...
class AddonRule extends Equatable {
  ...
  AddonRuleValidation validate(AddonCategory category) {
    ...
    if (minSelection > maxSelection) {
      errors.add(
        'Minimum selection ($minSelection) cannot exceed maximum ($maxSelection)',
      );
    }
    ...
    if (defaultItemIds.length > maxSelection) {
      errors.add(
        'Default items (${defaultItemIds.length}) exceeds maximum selection ($maxSelection)',
      );
    }

    if (defaultItemIds.length < minSelection) {
      errors.add(
        'Default items (${defaultItemIds.length}) is less than minimum selection ($minSelection)',
      );
    }
    ...
    for (final defaultId in defaultItemIds) {
      if (!categoryItemIds.contains(defaultId)) {
        errors.add(
          'Default item $defaultId not found in category ${category.name}',
        );
      }
    }
...
class ItemAddonAttachment extends Equatable {
  final String itemId;
  final String?
  sizeId; // null means item-level (all sizes), otherwise size-specific
