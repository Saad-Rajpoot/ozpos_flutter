...
Future<void> showEditPrinterDialog(
    BuildContext context, PrinterEntity printer) async {
  ...
  await showDialog(
    ...
    builder: (dialogContext) => BlocProvider(
      create: (_) => EditPrinterDialogCubit(printer: printer),
      child: BlocBuilder<EditPrinterDialogCubit, EditPrinterDialogState>(
        builder: (dialogContext, state) {
          final theme = Theme.of(dialogContext);
          final cubit = dialogContext.read<EditPrinterDialogCubit>();
          Widget content;
          switch (state.step) {
            case 0:
              content = Column(
          ...
          return AlertDialog(
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                child: Text(state.step < 4 ? 'Next' : 'Save Changes'),
              ),
            ],
          );
        },
...
class EditPrinterDialogState extends Equatable {
  const EditPrinterDialogState({
    required this.step,
  ...
  EditPrinterDialogState copyWith({
    int? step,
...
class EditPrinterDialogCubit extends Cubit<EditPrinterDialogState> {
  EditPrinterDialogCubit({required PrinterEntity printer})
      : super(EditPrinterDialogState(
          step: 0,
          selectedType: printer.type,
          selectedConnection: printer.connection,
          fnReceipts: true,
          fnTickets: true,
          fnReports: false,
        ));
