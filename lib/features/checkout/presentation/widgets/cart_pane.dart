...
class CartPane extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // User dropdown (only for delivery)
          if (state.orderType == OrderType.delivery)
            _buildUserSelector(context, state),

          // Customer search field (for takeaway)
          if (state.orderType == OrderType.takeaway)
  ...
  Widget _buildUserSelector(BuildContext context, CartLoaded state) {
    // Use the shared UserManagementBloc instance from app level
    // This ensures real-time updates when new users are added
    final userBloc = context.read<UserManagementBloc>();

    // Load users if not already loaded
    final currentUserState = userBloc.state;
    if (currentUserState is! UserManagementLoaded &&
        currentUserState is! UserManagementLoading) {
      userBloc.add(const LoadUsersEvent());
    }

    return BlocListener<UserManagementBloc, UserManagementState>(
      listener: (context, userState) {
        // When a user is updated, sync the cart's selected user if it matches
        if (userState is UserManagementLoaded && state.selectedUser != null) {
          final updatedUser = userState.users.firstWhere(
            (u) => u.id == state.selectedUser!.id,
            orElse: () => state.selectedUser!,
          );

          // If the user was updated, update the cart's selected user
      ...
      child: BlocBuilder<UserManagementBloc, UserManagementState>(
        builder: (context, userState) {
          ...
          bool isLoading = userState is UserManagementLoading;

          // Ensure ALL users are loaded completely
          if (userState is UserManagementLoaded) {
            users = userState.users.toList(); // Show all users
          }

          final isLoaded = userState is UserManagementLoaded;

          // Fixed height container to prevent visual shifts
          return SizedBox(
            height: 48, // Fixed height for visual stability
            child: isLoading
                ? Container(
                        ...
                        ? Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                ...
                                width: state.selectedUser == null ? 2 : 1,
                              ),
                              borderRadius: BorderRadius.circular(8),
                            ),
                            child: Row(
